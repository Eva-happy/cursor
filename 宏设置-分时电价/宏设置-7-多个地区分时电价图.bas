...
Sub ConvertToTimeConfig()
    ...
    ' 在颜色时段图后添加配置区域
    configStartRow = currentRow + 2 ' 留出两行空间
    
    ' 添加配置区域标题
    wsMulti.Cells(configStartRow, 1).value = "时段状态配置（1=尖峰,2=高峰,3=平段,4=低谷,5=深谷）："
    configStartRow = configStartRow + 1
    ...
    For Each region In regions
        For monthNum = 1 To 12
            ...
            ' 复制对应的时段配置
            For i = 0 To 23
                wsMulti.Cells(configRow, i + 3).value = wsMulti.Cells((regions.Count * (monthNum - 1)) + 2, i + 3).value
            Next i
...
' 新增：更新选定地区的数据到单个地区时段表
Sub UpdateSelectedRegion(selectedRegion As String)
    ...
    ' 设置地区标题
    wsSingle.Cells(1, 1).value = selectedRegion
    ...
    ' 遍历多个地区时段表中的数据
    For i = 2 To lastRow
        If Trim(wsMulti.Cells(i, 1).value) = selectedRegion Then
            ' 获取月份
            Dim monthStr As String
            monthStr = wsMulti.Cells(i, 2).value
            Dim monthNum As Integer
            monthNum = CInt(Replace(monthStr, "月", ""))
            
            ' 复制时段数据和颜色
            For j = 3 To 26 ' 对应0-23小时
                ' 复制数据到颜色时段图
                If Not IsEmpty(wsMulti.Cells(i, j)) Then
                    ' 复制数值到配置区域
                    wsSingle.Cells(monthNum + 16, j - 1).value = wsMulti.Cells(i, j).value
                    
                    ' 设置颜色时段图的颜色
                    Select Case wsMulti.Cells(i, j).value
                        Case PEAK ' 尖峰
                            wsSingle.Cells(monthNum + 1, j - 1).Interior.Color = RGB(255, 192, 0)  ' 橙色
                        Case HIGH ' 高峰
                            wsSingle.Cells(monthNum + 1, j - 1).Interior.Color = RGB(255, 192, 203)  ' 粉红色
                        Case NORMAL ' 平段
                            wsSingle.Cells(monthNum + 1, j - 1).Interior.Color = RGB(189, 215, 238) ' 浅蓝色
                        Case LOW ' 低谷
                            wsSingle.Cells(monthNum + 1, j - 1).Interior.Color = RGB(198, 239, 206) ' 浅绿色
                        Case DEEP_LOW ' 深谷
                            wsSingle.Cells(monthNum + 1, j - 1).Interior.Color = RGB(0, 112, 192)   ' 深蓝色
                    End Select
                End If
            Next j
        End If
...
Private Sub CreateMultiRegionTemplate(ws As Worksheet)
    ...
    ' 复制列标题到配置区域
    For i = 0 To 23
        ws.Cells(16, i + 3).value = "'" & i & "-" & (i + 1)
        ws.Cells(16, i + 3).HorizontalAlignment = xlCenter
    Next i
    
    ' 设置配置区域标题行格式
    With ws.Range("A16:Z16")
        .Font.Bold = True
        .HorizontalAlignment = xlCenter
        .VerticalAlignment = xlCenter
        .Interior.Color = RGB(217, 217, 217)  ' 浅灰色背景
    ...
    ' 设置配置区域边框
    With ws.Range(ws.Cells(16, 1), ws.Cells(28, 26))
        .Borders.LineStyle = xlContinuous
        .Borders(xlEdgeLeft).LineStyle = xlContinuous
        .Borders(xlEdgeTop).LineStyle = xlContinuous
        .Borders(xlEdgeBottom).LineStyle = xlContinuous
...
Private Sub UpdateSingleRegionData(ByVal wsSingle As Worksheet, ByVal wsSource As Worksheet, ByVal region As String, ByVal lastRow As Long)
    ' 清除现有数据
    wsSingle.Range("B2:Y13").ClearContents
    wsSingle.Range("B17:Y28").ClearContents
    wsSingle.Range("B2:Y13").Interior.ColorIndex = xlNone
    ...
    ' 为每个月份创建时段配置数组
    Dim monthConfigs(1 To 12, 0 To 23) As Integer
    ...
    For i = 2 To lastRow
        If Trim(wsSource.Cells(i, 1).value) = region Then
            Dim monthNum As Integer
            monthNum = CInt(Replace(wsSource.Cells(i, 3).value, "月", ""))
            ...
            For j = startHour To endHour - 1
                ...
                If targetHour >= 0 And targetHour <= 23 Then
                    ' 只有当新的时段类型优先级更高时才覆盖
                    If monthConfigs(monthNum, targetHour) = 0 Or timeValue < monthConfigs(monthNum, targetHour) Then
                        monthConfigs(monthNum, targetHour) = timeValue
                    End If
                End If
            Next j
        End If
    ...
    For i = 1 To 12
        For j = 0 To 23
            If monthConfigs(i, j) <> 0 Then
                ' 写入配置值
                wsSingle.Cells(i + 16, j + 2).value = monthConfigs(i, j)
                
                ' 设置颜色
                Select Case monthConfigs(i, j)
                    Case PEAK ' 尖峰
