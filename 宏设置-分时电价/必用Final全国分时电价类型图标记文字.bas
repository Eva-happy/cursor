...
Private Function ExtractTimePairs(text As String) As String()
    ...
    If pos > 0 Then
        ...
        timeStr = Replace(timeStr, "~", "-")   ' 替换波浪号
        timeStr = Replace(timeStr, "～", "-")  ' 替换全角波浪号
...
Sub ParseTimeText()
    ...
    ElseIf inputChoice = vbYes Then
        ...
        ' 记录开始处理
        ws.Cells(debugRow, 6).value = "开始处理'分时电价政策表'工作表的数据"
        debugRow = debugRow + 1
        ...
        For i = 2 To lastRow
            ...
            ' 记录原始行内容
            If line <> "" Then
                ws.Cells(debugRow, 6).value = "地区: " & currentRegion & " 行 " & i & ": " & line
                debugRow = debugRow + 1
                
                Call ProcessTextLine(line, currentMonths, specificMonths, timeType, ws, debugRow, currentRow, processedEntries, currentRegion)
            End If
        Next i
    Else
        ' 手动输入文本
        Do
            ...
            ' 记录原始文本
            ws.Cells(debugRow, 6).value = "新输入文本块开始 - 地区: " & currentRegion
            debugRow = debugRow + 1
...
Private Sub ProcessTextLine(ByVal line As String, ByRef currentMonths As String, _
    ...
    If matches.Count > 0 Then
        ...
        ' 移除所有括号和"月份："前缀
        monthPart = Replace(monthPart, "月份：", "")
        ...
        For Each monthGroup In monthGroups
            monthGroup = Trim(CStr(monthGroup))
            If monthGroup <> "" Then
...
' 新增：分析剩余时间段的宏
Sub AnalyzeRemainingTimeSlots()
    Dim inputText As String
    Dim timeSlots() As String
    Dim remainingSlots As Collection
    Set remainingSlots = New Collection
    ...
    For i = 0 To 95
        If Not timePoints(i) And Not isInGap Then
            ...
            currentStart = i
            isInGap = True
        ElseIf timePoints(i) And isInGap Then
            ' 找到空隙的结束
            remainingSlots.Add IndexToTime(currentStart) & "-" & IndexToTime(i)
            isInGap = False
        End If
    Next i
    ...
    ' 处理最后一个空隙
    If isInGap Then
        remainingSlots.Add IndexToTime(currentStart) & "-24:00"
    End If
...
Sub CreateHolidayTimeTable()
    ...
    If wsHoliday Is Nothing Then
        Set wsHoliday = ThisWorkbook.Worksheets.Add
        wsHoliday.Name = "国家节假日时段分析"
    End If
    wsHoliday.Tab.ColorIndex = 50 ' 设置为青绿色
    ...
    Dim i As Integer
    For i = 0 To 23
        wsHoliday.Cells(1, i + 2).value = "'" & i & "-" & (i + 1)
        wsHoliday.Cells(1, i + 2).HorizontalAlignment = xlCenter
        wsHoliday.Columns(i + 2).ColumnWidth = 5
    Next i
    ...
    ' 添加图例说明
    wsHoliday.Cells(2, 28).value = "图例说明："
    wsHoliday.Cells(3, 28).value = "尖峰时段"
    wsHoliday.Cells(4, 28).value = "高峰时段"
    wsHoliday.Cells(5, 28).value = "平时段"
    wsHoliday.Cells(6, 28).value = "低谷时段"
    wsHoliday.Cells(7, 28).value = "深谷时段"
    
    ' 设置图例颜色
    wsHoliday.Cells(3, 29).Interior.color = RGB(255, 192, 0)   ' 橙色 - 尖峰
    wsHoliday.Cells(4, 29).Interior.color = RGB(255, 192, 203) ' 粉红色 - 高峰
    wsHoliday.Cells(5, 29).Interior.color = RGB(189, 215, 238) ' 浅蓝色 - 平段
    wsHoliday.Cells(6, 29).Interior.color = RGB(198, 239, 206) ' 浅绿色 - 低谷
    wsHoliday.Cells(7, 29).Interior.color = RGB(0, 112, 192)   ' 深蓝色 - 深谷
    
    ' 添加配置说明
    wsHoliday.Cells(15, 1).value = "时段状态配置（1=尖峰,2=高峰,3=平段,4=低谷,5=深谷）："
    
    ' 复制列标题到配置区域
    For i = 0 To 23
        wsHoliday.Cells(16, i + 2).value = "'" & i & "-" & (i + 1)
        wsHoliday.Cells(16, i + 2).HorizontalAlignment = xlCenter
    Next i
    
    ' 设置表格边框
    With wsHoliday.Range(wsHoliday.Cells(1, 1), wsHoliday.Cells(13, 25))
        .Borders.LineStyle = xlContinuous
        .Borders(xlEdgeLeft).LineStyle = xlContinuous
        .Borders(xlEdgeTop).LineStyle = xlContinuous
        .Borders(xlEdgeBottom).LineStyle = xlContinuous
    ...
    ' 添加按钮
    Dim btnUpdate As Button
    Set btnUpdate = wsHoliday.Buttons.Add(wsHoliday.Range("AA10").Left, wsHoliday.Range("AA10").Top, 80, 25)
    With btnUpdate
        .OnAction = "UpdateHolidayTimeTable"
        .Caption = "更新时段表"
    End With
    
    Dim btnChart As Button
    Set btnChart = wsHoliday.Buttons.Add(wsHoliday.Range("AA13").Left, wsHoliday.Range("AA13").Top, 120, 25)
    With btnChart
        .OnAction = "'" & ThisWorkbook.Name & "'!CreateHolidayTimeCharts"
        .Caption = "生成分时电价柱状图"
    End With
...
Sub Create2025HolidaySheet()
    ...
    Set btnAnalyze = ws.Buttons.Add(ws.Range("P1").Left, ws.Range("P1").Top, 120, 25)
    With btnAnalyze
    ...
    Set btnParseHoliday = ws.Buttons.Add(ws.Range("P3").Left, ws.Range("P3").Top, 120, 25)
    ...
    Set btnCreateHolidayTable = ws.Buttons.Add(ws.Range("P5").Left, ws.Range("P5").Top, 120, 25)
